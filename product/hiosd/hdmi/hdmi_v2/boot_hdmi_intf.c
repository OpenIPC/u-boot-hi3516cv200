...
do{\
    if (HI_HDMI_VIDEO_MODE_RGB444 == uvideomode)\
    {\
        kcolorspace = HDMI_COLORSPACE_RGB;\
    }\
    else if (HI_HDMI_VIDEO_MODE_YCBCR422 == uvideomode)\
    {\
        kcolorspace = HDMI_COLORSPACE_YCbCr422;\
    }\
    else if (HI_HDMI_VIDEO_MODE_YCBCR444 == uvideomode) \
    {\
...
static HI_S32 HdmiModeSchedule(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    if (pstVideoAttr->u32HdmiAdaptPixClk >= HDMI_MAX_HDMI14_TMDS_RATE)
    {
        ...
        /* @2016.02.22, when the u32HdmiAdaptPixClk is bigger than 340 at 10bit or 12bit,
...
HI_S32 HI_DRV_HDMI_SetAttr(HI_UNF_HDMI_ID_E enHdmiID, HI_DRV_HDMI_ATTR_S *pstAttr)
{
    ...
    HDMI_UVIDEOMODE_2_KCOLORSPACE(pstAttr->enVidOutMode, stAttr.stAppAttr.enOutColorSpace);
    HDMI_UVIDEOMODE_2_KCOLORSPACE(pstAttr->enVidInMode, stAttr.stVOAttr.enInColorSpace);
