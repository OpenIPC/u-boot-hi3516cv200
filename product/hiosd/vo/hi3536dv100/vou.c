...
static inline HI_U32 Rgb2Yuv_full(HI_U32 u32Rgb)
{
    HI_U8 r,g,b;
    HI_U8 y,u,v;
    ...
    y = MAX2(MIN2(py_temp, 255), 0);
    u = MAX2(MIN2(pcb_temp, 255), 0);
    v = MAX2(MIN2(pcr_temp, 255), 0);
...
int start_vo(unsigned int dev, unsigned int type, unsigned int sync)
{
    ...
    VOU_DRV_SetDevBkGrd(dev, Rgb2Yuv_full(g_au32BgColor[dev]));
    ...
    unsigned int x, unsigned int y, unsigned int w, unsigned int h)
{
    ...
    HAL_LAYER_SetCscMode(enLayer,HI_TRUE);
    ...
    unsigned int x, unsigned int y, unsigned int w, unsigned int h)
{
    ...
    HAL_VIDEOLAYER_SetCscMode(layer,HAL_CSC_MODE_BT709FULL_TO_BT709FULL);
