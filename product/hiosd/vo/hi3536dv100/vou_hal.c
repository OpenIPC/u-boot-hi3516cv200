...
#define CRG_PERCTL4_ADDR (0x0008 + CRG_REGS_ADDR)
#define CRG_PERCTL5_ADDR (0x000C + CRG_REGS_ADDR)



#define CRG_PERCTLVDPCRG_ADDR (0x0088 + CRG_REGS_ADDR)
...
HI_S32 SYS_HAL_VouHdDacClkEn(HI_BOOL pbClkEn)
{
    ...
    HI_RegSetBit(u32Tmp, 2, IO_ADDRESS(CRG_PERCTLVDPCRG_ADDR));
    return 0;
}
...
{
    HI_RegSetBit(u32ClkSel, 6, IO_ADDRESS(CRG_PERCTLVDPCRG_ADDR));
    return 0;
}
...
HI_S32 SYS_HAL_VouBusResetSel(HI_BOOL bReset)
{
    ...
    HI_RegSetBit(u32Tmp, 0, IO_ADDRESS(CRG_PERCTLVDPCRG_ADDR));
...
HI_S32 SYS_HAL_VouBusClkEn(HI_BOOL pbClkEn)
{
    ...
    HI_RegSetBit(u32Tmp, 3, IO_ADDRESS(CRG_PERCTLVDPCRG_ADDR));
...
HI_S32 SYS_HAL_VouDevClkEn(HI_S32 s32VoDev, HI_BOOL pbClkEn)
{
    ...
    {
        u32Bit = 4 + s32VoDev;
        HI_RegSetBit(u32Tmp, u32Bit, IO_ADDRESS(CRG_PERCTLVDPCRG_ADDR));
    }
    else
    {
        return -1;
...
HI_S32 SYS_HAL_VouWorkClkEn(HI_U32 u32ClkSel)
{
    ...
    HI_RegSetBit(u32ClkSel, 5, IO_ADDRESS(CRG_PERCTLVDPCRG_ADDR));
    return 0;
}
...
HI_S32 SYS_HAL_VouPLLClkDiv(HI_S32 s32Pll, HI_U32 u32BitsSet)
{
    if (0 == s32Pll)
    {
        HI_RegWrite32(u32BitsSet << 12, 0x1f << 12, IO_ADDRESS(CRG_PERCTLVDPCRG_ADDR));
    }
    else if(1 == s32Pll)
    {
        return -1;
...
{	
    ...
    switch(enLayer)
    {
        ...
        {
            RegAbsAddr = (pReg) + (enLayer + 3) * VHD_REG_LEN;
            break;
        }
        ...
        {
            RegAbsAddr = ((pReg)) + (enLayer - 2) * GFX_REG_LEN;
            break;
        }
...
HI_U32 Vou_GetChnAbsAddr(HAL_DISP_OUTPUTCHANNEL_E enChan, HI_U32 pReg)
{
    ...
    switch(enChan)
    {
        ...
        default:
        {
            HAL_PRINT("Error channel id found in %s: L%d enChan %d\n",__FUNCTION__, __LINE__, enChan);
            return 0;
        }
    }
     return RegAbsAddr;
...
HI_U32 Vou_GetIntfAbsAddr(HAL_DISP_INTF_E enIntf, HI_U32 pReg)
{
    ...
    switch(enIntf)
    {
        ...
        default:
        {
            HAL_PRINT("Error intf id found in %s: L%d enIntf %d\n",__FUNCTION__, __LINE__, enIntf);
            return 0;
        }
    }
     return RegAbsAddr;
...
HI_BOOL  HAL_DISP_SetIntfMuxSel(HAL_DISP_OUTPUTCHANNEL_E enChan,HAL_DISP_INTF_E enIntf)
{
    ...
    switch(enIntf)
    {
        ...
        {
            VO_MUX.bits.bt1120_sel = enChan+1;
            break;
        }
        case HAL_DISP_INTF_HDMI:
        {
            VO_MUX.bits.hdmi_sel = enChan+1;
            break;
        }
        case HAL_DISP_INTF_VGA:
        {
            VO_MUX.bits.vga_sel = enChan+1;
            break;
        }
        case HAL_DISP_INTF_CVBS:
        {
...
HI_BOOL HAL_LAYER_SetCscMode(HAL_DISP_LAYER_E enLayer, HI_BOOL bIsHCModeBy709)
{
    ...
    if((HAL_DISP_LAYER_HC0 == enLayer) ||(HAL_DISP_LAYER_GFX4 == enLayer))
    {
        addr_REG = Vou_GetAbsAddr(enLayer,(HI_U32)&(pVoReg->G0_CSC_IDC.u32));
        G0_CSC_IDC.u32 = HAL_ReadReg((HI_U32*)addr_REG);
        if (HI_TRUE == bIsHCModeBy709)
        {
            //G0_CSC_IDC.bits.csc_mode = 0x7;
            G0_CSC_IDC.bits.csc_mode = 0xC; /* 1100: RGB2YUV_709full */
        }
        else
        {
            //G0_CSC_IDC.bits.csc_mode = 0x6; /* 110:RGB2YUV_601 ,can't enter here*/
            return HI_FALSE;
        }
        HAL_WriteReg((HI_U32*)addr_REG, G0_CSC_IDC.u32); 
    }
...
HI_BOOL HAL_VIDEOLAYER_SetCscMode(HAL_DISP_LAYER_E enLayer, HAL_CSC_MODE_E enCscMode)
{
    ...
    if((HAL_DISP_LAYER_VHD1 == enLayer) ||(HAL_DISP_LAYER_VP == enLayer))
    {
        addr_REG = Vou_GetAbsAddr(enLayer,(HI_U32)&(pVoReg->V0_CSC_IDC.u32));
        V0_CSC_IDC.u32 = HAL_ReadReg((HI_U32*)addr_REG);
        if (HAL_CSC_MODE_BT601FULL_TO_BT709FULL == enCscMode)
        {
            V0_CSC_IDC.bits.csc_mode = 0xB; /* 1011:BT601FULL_TO_BT709FULL */
        }
        else if (HAL_CSC_MODE_BT709FULL_TO_BT709FULL == enCscMode)
        {
            V0_CSC_IDC.bits.csc_mode = 0xA; /* 1010:BT709FULL_TO_BT709FULL */
        }
        else if (HAL_CSC_MODE_BT601LIMIT_TO_BT709FULL == enCscMode)
        {
            V0_CSC_IDC.bits.csc_mode = 0x9; /* 1001:BT601LIMIT_TO_BT709FULL */
        }
        else if (HAL_CSC_MODE_BT709LIMIT_TO_BT709FULL == enCscMode)
        {
            V0_CSC_IDC.bits.csc_mode = 0x8; /* 1000:BT709LIMIT_TO_BT709FULL */
        }
        else
        {
            //can't enter here*/
